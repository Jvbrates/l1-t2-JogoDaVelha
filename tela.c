...
void tela_inicializa_cores(void)
{
  ...
  cores[fundo]         = al_map_rgb(13, 17, 23);;
  cores[fundo_t]     = al_map_rgb(22, 27, 34);//;
  cores[roxo]        = al_map_rgb(136, 57, 239);
  cores[branco]      = al_map_rgb(239, 241, 245);
  cores[amarelo]        = al_map_rgb(223, 142, 29);
  cores[vermelho]      = al_map_rgb(210, 15, 57);
  cores[azul]         = al_map_rgb(32, 159, 181);
  cores[preto]       = al_map_rgb(0, 0, 0);
  cores[verde]       = al_map_rgb_f(0, 1, 0);
}


int tela_cria_cor_melhor(int vm, int az, int vd){
  return tela_cria_cor((float)vm/255, (float)vm/255, (float)vm/255);
}
...
void tela_prepara_fonte(int tam)
{
  ...
  fonte = al_load_font("Arcade.ttf", tam, 0);
  if (fonte == NULL) {
    al_uninstall_system();
    printf("\n\nERRO FATAL\n");
